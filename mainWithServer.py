...
thread = server.threading.Thread(target=server.start)
thread.start()
...
class Screen_begin(Screen):
	def __init__(self, **kwargs):
		...
		self.image = Image(source='./Assets/game_begin.png', keep_ratio=False, allow_stretch=True, pos=(0,0),size_hint=(None,None), size=(x_0,y_0))
...
class GreenPipes(Widget):
	...
	def __init__(self,bird, **kwargs):
		...
		for i in range(6):
			...
			self.Pipe = Image(source="./Assets/down_pipe.png", keep_ratio=False,allow_stretch=True,pos=(self.x + (x_0*0.5*i), self.y),size_hint=(None, None), size=(self.size_x, size_y_below))
			self.Pipe_ = Image(source="./Assets/up_pipe.png", keep_ratio=False,allow_stretch=True,pos=(self.x + (x_0*0.5*i), y_0-(y_0 - (2*size_y_below))), size_hint=(None, None) ,size = (self.size_x, y_0 - (2*size_y_below)))
...
class Bird(Widget):
	def __init__(self, **kwargs):
		...
		self.Bird_ = Image(source="./Assets/bird_down.png",keep_ratio=False,allow_stretch=True,pos=(self.x, self.y-(self.y_size/2)), size_hint= (None, None), size=(x_0//10, y_0//20))
	...
	def update(self, dt):
		...
		if self.v >= 0:
			self.Bird_.source = "./Assets/bird_up.png"
		else:
			self.Bird_.source = "./Assets/bird_down.png"
...
class FlappyGame(Widget):
	def __init__(self, **kwargs):
		...
		self.add_widget(Image(source='./Assets/backdrop.png', keep_ratio= False, allow_stretch=True, size_hint=(None,None), size=(x_0, y_0)))
		...
		Clock.schedule_interval(self.Hop_update, 1/60)

	def score_update(self, dt):
		self.Score.text = '    Score: '+str(int(self.bird.score))

	def Hop_update(self, dt):
    		if 'Hop!' == server.message_:
    				Clock.schedule_once(self.server_message, 1)
    				self.bird.keyboard()


	def server_message(self, dt):
    		server.message_ = 0
